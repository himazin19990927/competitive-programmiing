...
void test_operator(vec3 v1, vec3 v2, float a) {
    ...
    std::cout << v1 << " - " << v2 << " = " << v1 - v2 << std::endl;
    std::cout << a << " * " << v2 << " = " << a * v2 << std::endl;
    std::cout << v1 << " * " << a << " = " << v1 * a << std::endl;
}

void test_translate(vec3 v1) {
    std::cout << "v1.transform(identity) = " << v1.transform(mat4::identity())
              << std::endl;
    std::cout << "v1.transform(translate(10, 100, 1000)) = "
              << v1.transform(mat4::translate(10, 100, 1000)) << std::endl;
}

void test_rotation(vec3 v1) {
    std::cout << "v1.transform(rotateX(pi/2) = "
              << v1.transform(mat4::rotate_axis_x(M_PI / 2)) << std::endl;
    std::cout << "v1.transform(rotateY(pi/2) = "
              << v1.transform(mat4::rotate_axis_y(M_PI / 2)) << std::endl;
    std::cout << "v1.transform(rotateZ(pi/2) = "
              << v1.transform(mat4::rotate_axis_z(M_PI / 2)) << std::endl;
}
...
int main() {
    ...
    std::cout << "result = "
              << vec3().transform(mat4::translate(2, 1, 0) *
